...
function CodeforcesProblemsList() {
  ...
  useEffect(() => {
    ...
    const stringSHA = `123456/user.status?apiKey=${apikey}&handle=kunal_91&time=${timenow}#${secretkey}`;
    const generatedSHA = generateSHA512(stringSHA);

    const fetchData = async () => {
      try {
        const response = await fetch(
          `https://codeforces.com/api/user.status?handle=kunal_91&apiKey=${apikey}&time=${timenow}&apiSig=123456${generatedSHA}`
        );
        if (!response.ok) {
  ...
  useEffect(() => {
    if (data !== null) {
      ...
      const problems1500 =
        (problemRatingMap.get(1600) || 0) + (problemRatingMap.get(1500) || 0);
      const problems1700 =
        (problemRatingMap.get(1700) || 0) + (problemRatingMap.get(1800) || 0);
      const problems1900 =
        (problemRatingMap.get(1900) || 0) + (problemRatingMap.get(2000) || 0);
